...
MuseScore {
	...
	// Metadata must be declared as top-level properties so MuseScore
	// can read it before the plugin is executed.
	title: "IEEE1599 Converter"
	menuPath: "Plugins.Export to IEEE 1599"
	version: "4.0"
	description: "Export to IEEE 1599 format"
	pluginType: "dialog"
	thumbnailName: "ieee1599musescore.png"
	categoryCode: "Conversion"
	width: 800
	height: 600
	
	// Global colors
    property color bgColor: "#ffb347"      // orange
    property color textColor: "#404040"    // dark gray
    ...
    function scoreDirectory() {
        // Prefer the directory of the currently opened score. This is user-independent
        // and works on Windows, macOS and Linux when the score has been saved.
        try {
            if (curScore) {
                if (curScore.path)
                    return directoryName(curScore.path)
                if (curScore.filePath)
                    return directoryName(curScore.filePath)
            }
        } catch (e) {
            console.log("Cannot read current score path: " + e)
        }

        return ""
    }

    function conventionalMuseScoreScoresDirectory() {
        // Strategy 2: use MuseScore's conventional Scores folder.
        // FileIO.homePath() is provided by the MuseScore FileIO plugin API and
        // returns the current user's home directory, avoiding hard-coded usernames.
        try {
            var home = xmlWriter.homePath()
            if (home) {
                home = home.toString().replace(/\\/g, "/")
                return home + "/Documents/MuseScore4/Scores"
            }
        } catch (e) {
            console.log("Cannot read FileIO.homePath(): " + e)
        }
    ...
    function defaultExportDirectory() {
        // Prefer the conventional MuseScore user Scores folder.
        var conventionalDir = conventionalMuseScoreScoresDirectory()
        if (conventionalDir !== "")
            return conventionalDir

        // Fallback: use the current score folder, if the score has already been saved.
	...
	// Metadata is declared above, not in Component.onCompleted, because MuseScore
	// reads title, categoryCode and thumbnailName before executing the plugin.
	
	Component.onDestruction: {
		settings.exportDirectory = textFieldFilePath.text
	}
	...
	onRun: {
        ...
        // At component construction time curScore may not yet be available.
        // Compute the default export directory again when the plugin is actually run.
        if (!textFieldFilePath.text)
            textFieldFilePath.text = defaultExportDirectory()
...
FileIO {
		id: xmlWriter
		onError: function(msg) { console.log(msg + "  Filename = " + xmlWriter.source) }
	}
...
// File names -------------------------------------------------
	...
	Label {
		...
		anchors.topMargin: smallSpace
		anchors.leftMargin: smallSpace
		width: smallWidth
		height: bigHeight
		verticalAlignment: Text.AlignVCenter
	}
	...
	TextField {
		...
		placeholderText: qsTr("default: MuseScore4/Scores; edit if needed")
		anchors.top: labelFileName.bottom
		anchors.left: labelFilePath.right
		anchors.topMargin: smallSpace
		anchors.leftMargin: smallSpace
        text: ""
		width: bigWidth - buttonWidth - smallSpace
		height: stdHeight
        readOnly: false
        selectByMouse: true
	}
    ...
    IeeeButton {
        ...
        MouseArea {
            anchors.fill: parent
            cursorShape: Qt.PointingHandCursor
            onClicked: textFieldFilePath.text = defaultExportDirectory()
        }
                palette.buttonText:"black"
...
RadioButton {
			...
			text: "chords"
			checked: true
			ButtonGroup.group: groupBoxMappingAround
		}
		RadioButton {
			id: radioButtonNotes
			text: "notes"
	...
	IeeeButton {
		...
		MouseArea {
			anchors.fill: parent
            onClicked: textFieldDPI.text = qsTr("360")
		}
				palette.buttonText:"black"
	...
	IeeeButton {
		...
		MouseArea {
			anchors.fill: parent
            onClicked: textFieldDistance.text = qsTr("1.750")
		}
				palette.buttonText:"black"
